...
read_in_and_filter2<-function(obs_path,kml){
  ...
  kalb$vouchered<-case_when(kalb$basisOfRecord == "PreservedSpecimen" |
  ...
  kalb$specificEpithet <- sub("×", "", kalb$specificEpithet) # Direct character
  kalb$specificEpithet <- sub("\u00D7", "", kalb$specificEpithet) # Unicode escape
