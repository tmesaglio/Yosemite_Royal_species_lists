...
Yosemite_species_list %>% names() %>% as.vector() -> names_to_use

Yosemite_species_list %>% 
  select(accepted_name, scientific_name, taxonomic_status, family, year_first_voucher, year_last_voucher, iNat_year_first, iNat_year_last) %>%
  select(-year_first_voucher, -year_last_voucher, -iNat_year_first, -iNat_year_last) %>%
  left_join(
  Yosemite_first_last_data %>% 
    select(accepted_name, 
           year_first_voucher = first_voucher,
           year_last_voucher = last_voucher,
           iNat_year_first = first_iNat,
           iNat_year_last = last_iNat)) %>% 
  mutate(
    year_first_voucher = ifelse(taxonomic_status!="accepted", NA,year_first_voucher),
    year_last_voucher = ifelse(taxonomic_status!="accepted", NA,year_last_voucher),
    iNat_year_first = ifelse(taxonomic_status!="accepted", NA,iNat_year_first),
    iNat_year_last = ifelse(taxonomic_status!="accepted", NA,iNat_year_last),
    ) %>%
  select(any_of(names_to_use)) %>% View() 
