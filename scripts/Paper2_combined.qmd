...
Yosemite_species_list <- read_csv("data_Yosemite/Yosemite_all_data_2025_03_16.csv")
Yosemite_species_list_extra <- Yosemite_species_list
...
Yosemite_manual_dates <- read_csv("data_Yosemite/Yosemite_manual_dates.csv")

Yosemite_CCH2_data <- read_csv("data_Yosemite/Yosemite_CCH_records_2024_09_22.csv") %>%
...
Yosemite_recent_additions_all <- read_csv("data_Yosemite/Yosemite_recent_additions_all.csv")
...
cowplot::save_plot("outputs/maps.svg", all_maps, base_width = 8, base_height = 9)
```

## Figure 3. First and last detections
```{r}
Yosemite_iNat_first_last <- Yosemite_iNat %>%
  mutate(year = year(observed_on)) %>%
  select(scientific_name, year) %>%
  arrange(scientific_name, year) %>%
  group_by(scientific_name) %>%
    mutate(
      first_iNat = first(year),
      last_iNat = last(year)
    ) %>%
  ungroup() %>%
  filter(!is.na(scientific_name)) %>%
  distinct(scientific_name, first_iNat, last_iNat) %>%
  mutate(last_iNat = ifelse(is.na(last_iNat), first_iNat, last_iNat))

Yosemite_voucher_first_last <- Yosemite_voucher_data %>%
  rename(scientific_name = current_name) %>%
  mutate(scientific_name = ifelse(
    word(scientific_name, start = 2, end = 2) == "x", 
    word(scientific_name, start = 1, end = 3), 
    word(scientific_name, start = 1, end = 2))
    ) %>%
  select(scientific_name, year) %>%
  ...
  group_by(scientific_name) %>%
    mutate(
      first_voucher = first(year),
      last_voucher = last(year)
    ) %>%
  ungroup() %>%
  distinct(scientific_name, first_voucher, last_voucher) %>%
  mutate(last_voucher = ifelse(is.na(last_voucher), first_voucher, last_voucher))

Yosemite_first_last_data <- 
  Yosemite_species_paper_2 %>% 
  select(accepted_name, establishment_means, final_list, iNat_RG_END, voucher_END, old_voucher, recent_voucher) %>%
  left_join(Yosemite_iNat_first_last %>% rename(accepted_name = scientific_name)) %>%
  left_join(Yosemite_voucher_first_last %>% rename(accepted_name = scientific_name)) %>%
  left_join(Yosemite_manual_dates) %>%
  mutate(
    first_iNat = ifelse(is.na(first_iNat), first_iNat_manual, first_iNat),
    last_iNat = ifelse(is.na(last_iNat), last_iNat_manual, last_iNat)
  ) %>%
  # this line needs to get commented out for testing that everything is picked up correctly
  mutate(
    first_voucher = ifelse(is.na(voucher_END), NA, first_voucher),
    last_voucher = ifelse(is.na(voucher_END), NA, last_voucher)
  )
  
  # tests
  # taxa in iNat at the end all have iNat years - should be 0
  Yosemite_first_last_data %>% filter(iNat_RG_END == 1) %>% filter(is.na(first_iNat))
  # taxa not in iNat at the end don't have iNat years - should be 0
  Yosemite_first_last_data %>% filter(is.na(iNat_RG_END)) %>% filter(!is.na(first_iNat))
  # taxa with vouchers at the end all have voucher years - should be 2, because only Triticum aestivum undated; and Dean Taylor mentions Boechera pendulocarpa vouchers that aren't in CCH/gbif
  Yosemite_first_last_data %>% filter(voucher_END == 1) %>% filter(is.na(first_voucher))
  # taxa with vouchers at the end don't have voucher years - should be 1, Ranunculus repens; this taxon appears on the list because there is a valid iNat record, but the voucher has been discounted as a wrong ID; so in CCH file still a "keep", because geographically correct and therefore a year range is generated. Taxa in this category will be removed by the final mutate in the code above.
  Yosemite_first_last_data %>% filter(is.na(voucher_END)) %>% filter(!is.na(first_voucher))

  Yosemite_first_last_data %>% filter(duplicated(accepted_name))
  
  Yosemite_first_last_data %>% write_csv("data_Yosemite/Yosemite_first_last_data.csv")
```

## Figure 4. Decline in "unobserved" over time

### - function to calculate declining long-unseens

- although the function isn't actually being used right now...
...
combined_plot_a <- ggplot() + 
  ...
  geom_rect(aes(xmin = 2003, xmax = 2025, ymin = 0, ymax = min(Royal_iNat_redetection_summary$remaining)), fill = "#74AC00", colour = NA, alpha = 0.1) + 
  geom_ribbon(aes(x = year, ymin = min(Royal_iNat_redetection_summary$remaining), ymax = remaining), data = Royal_iNat_redetection_summary %>% filter(year > 2002), fill = "#74AC00", color = "#74AC00", alpha = 0.35) +
...
combined_plot_c <- ggplot() + 
  ...
  geom_rect(aes(xmin = 2003, xmax = 2024, ymin = 0, ymax = min(Yosemite_iNat_redetection_summary$remaining)), fill = "#74AC00", colour = NA, alpha = 0.1) + 
  ...
  geom_ribbon(aes(x = year, ymin = min(Yosemite_iNat_redetection_summary$remaining), ymax = remaining), data = Yosemite_iNat_redetection_summary %>% filter(year > 2002), fill = "#74AC00", color = "#74AC00", alpha = 0.35) +
  scale_x_continuous(limits = c(1990, 2025), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
...
cowplot::save_plot("outputs/Fig4_decline_in_unobserved.jpg", decline_in_unobserved, base_width = 8, base_height = 6)
...
cowplot::save_plot("outputs/Fig4_decline_in_unobserved_v2.jpg", decline_in_unobserved, base_width = 12, base_height = 6)
...
combined_plot_a1 <- ggplot() +  
  geom_ribbon(aes(x = year, ymin = 0, ymax = remaining), data = Royal_voucher_redetection_summary, fill = "#608CB8", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining), data = Royal_voucher_redetection_summary, color = "#608CB8", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1000), breaks = c(200, 400, 600, 800, 1000), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 250, label = "status unknown", fontface = "italic")) +
...
combined_plot_a2 <- ggplot() + 
  geom_ribbon(aes(x = year, ymin = 0, ymax = remaining), data = Royal_iNat_redetection_summary, fill = "#74AC00", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining), data = Royal_iNat_redetection_summary, color = "#74AC00", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1000), breaks = c(200, 400, 600, 800, 1000), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 205, label = "status unknown", fontface = "italic")) +
...
combined_plot_b <- ggplot() + 
    geom_ribbon(aes(x = year, ymin = 0, ymax = remaining_combined), data = Royal_recent_obs_2_lines, fill = "#6EA387", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining_combined), data = Royal_recent_obs_2_lines, color = "#6EA387", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1000), breaks = c(200, 400, 600, 800, 1000), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 180, label = "status unknown", fontface = "italic")) +
...
combined_plot_c1 <- ggplot() + 
  geom_ribbon(aes(x = year, ymin = 0, ymax = remaining), data = Yosemite_voucher_redetection_summary, fill = "#608CB8", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining), data = Yosemite_voucher_redetection_summary, color = "#608CB8", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1600), breaks = c(500, 1000, 1500), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 220, label = "status unknown", fontface = "italic")) +
...
combined_plot_c2 <- ggplot() + 
  geom_ribbon(aes(x = year, ymin = 0, ymax = remaining), data = Yosemite_iNat_redetection_summary, fill = "#74AC00", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining), data = Yosemite_iNat_redetection_summary, color = "#74AC00", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1600), breaks = c(500, 1000, 1500), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 340, label = "status unknown", fontface = "italic")) +
...
combined_plot_d <- ggplot() + 
    geom_ribbon(aes(x = year, ymin = 0, ymax = remaining_combined), data = Yosemite_recent_obs_2_lines, fill = "#6EA387", alpha = 0.35) +
  geom_line(aes(x = year, y = remaining_combined), data = Yosemite_recent_obs_2_lines, color = "#6EA387", size = 0.5) +
  scale_x_continuous(limits = c(1990, 2024), breaks = c(1990, 2000, 2010, 2020), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 1600), breaks = c(500, 1000, 1500), expand = c(0,0)) +
  #geom_text(aes(x = 2008, y = 110, label = "status unknown", fontface = "italic")) +
...
cowplot::save_plot("outputs/Fig4_decline_in_unobserved_v3.jpg", decline_in_unobserved, base_width = 12, base_height = 6)
...
```{r}
iNat_20240901 <- read_csv("data_Yosemite_archives/iNat_Yos_RG_2024.09.01.csv")
...
```{r}
iNat_needsID_20240901 <- read_csv("data_Yosemite_archives/iNat_Yos_needsID_2024.09.01.csv")
...
# merge collectors into Royal voucher data
```{r}
royal_all_vouchers_curated <- read_csv("data_Royal/royal_all_vouchers_curated.csv")
records_with_collectors <- read_csv("data_Royal/records-2025-02-13.csv") %>% 
  select(species, day, month, year, recordedBy) %>%
  distinct() %>%
  filter(!is.na(recordedBy)) %>%
  group_by(species, day, month, year) %>%
  mutate(recordedBy = paste0(recordedBy, collapse = "; ")) %>%
  ungroup() %>%
  distinct() 

royal_all_vouchers_curated %>%
  left_join(records_with_collectors, by = c("species", "year", "month", "day")) %>%
  write_csv("data_Royal/royal_all_vouchers_curated_with_authors.csv")
```

# Paper 2, Supplementary Material - table of new arrivals and long unobserved (Royal)
```{r}
Royal_Supplementary_Table <- read_csv("data_Royal/Royal_Supplementary Table.csv")
Royal_coordinates <- read_csv("data_Royal/Royal_locations_first_last_coordinates_20241105.csv")

Royal_Supplementary_Table_paper2 <- Royal_Supplementary_Table %>%
  ...
  select(1:6, sublist, first_or_last_record_type, year_first_record, year_last_record, year_first_voucher, year_last_voucher, year_first_iNat_obs = date_first_iNat_obs, year_last_iNat_obs = date_last_iNat_obs, latitude, longitude) %>%
  ungroup() %>%
  mutate(
  ...
  write_csv("data_Royal/Royal_Supplementary_Table_paper2.csv", na = "")
```

# Paper 2, Supplementary Material - table of new arrivals and long unobserved (Yosemite)
```{r}
Yosemite_Supplementary_Table <- read_csv("data_Yosemite/Yosemite_all_data_2025_03_16.csv")
Yosemite_coordinates1 <- read_csv("data_Yosemite/long_unseen_edited_lat_lon.csv") %>% select(scientific_name = current_name, latitude, longitude) %>% mutate(first_or_last_record_type = "voucher")

# `Yosemite_recent_additions_all` created higher in script

Yosemite_recent_additions_all <- read_csv("data_Yosemite/Yosemite_recent_additions_all.csv")

Yosemite_long_unseen_locations2 <- Yosemite_long_unseen_locations %>% select(scientific_name = current_name, first_or_last_record_type = source, latitude, longitude)

Yosemite_coordinates <- Yosemite_recent_additions_all %>% select(scientific_name, first_or_last_record_type = source, latitude, longitude) %>% bind_rows(Yosemite_long_unseen_locations2)
...
Yosemite_Supplementary_Table_paper2 <- Yosemite_Supplementary_Table %>%
  ...
  left_join(Yosemite_coordinates) %>% 
  select(1:6, sublist, first_or_last_record_type, year_first_record, year_last_record, year_first_voucher, year_last_voucher, year_first_iNat_obs = year_first_iNat, year_last_iNat_obs = year_last_iNat, latitude, longitude) %>%
  ungroup() %>%
  mutate(
    first_record_type = ifelse(year_first_voucher < year_first_iNat_obs | is.na(year_first_iNat_obs), "voucher", "iNat"),
    first_record_type = ifelse(is.na(year_first_voucher), "iNat", first_record_type),
  ...
  select(-latitude, -longitude, -first_or_last_record_type) %>% 
  arrange(sublist, year_first_record) %>%
  write_csv("data_Yosemite/Yosemite_Supplementary_Table_paper2.csv", na = "")
```

```{r}
Yosemite_Supplementary_Table_paper2 %>% 
  filter(sublist != "old and recent records") %>% 
  filter(is.na(latitude_first_record_recent_discovery) & is.na(latitude_last_record_long_unobserved)) -> need_coord
```
