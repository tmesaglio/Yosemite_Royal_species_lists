...
```{r}
Table_S1 <- Yos_data %>%
  dplyr::select(dplyr::all_of(c("accepted_name", "scientific_name", "taxonomic_status", "family", "broader", "establishment_means", "count_RG", "in_park_START", "voucher_START", "iNat_RG_START", "in_park_END", "voucher_END", "iNat_RG_END", "old_voucher", "recent_voucher", "final_list", "parks_outcome", "parks_outcome_detailed", "voucher_outcome", "voucher_outcome_detailed", "iNat_outcome", "iNat_outcome_detailed", "year_first_voucher", "year_last_voucher", "date_first_iNat_obs", "date_last_iNat_obs", "count_needs_id", "iNat_needsID_only", "voucher_obs", "voucher_obs_to_1990", "voucher_obs_coord", "first_voucher_collector", "first_iNat_observer", "last_iNat_observer", "year_first_iNat", "year_last_iNat", "vouchers_from_gbif_only", "iNat_needs_ID_only_verdict", "Symbiota", "iNat_missing_in_gbif", "use_as_example", "voucher_parklist_notes", "iNat_notes", "range_notes", "voucher_links", "species added during manual voucher searches", "iNat_needsID_only_notes", "iNat_needsID_only_notes2", "taxonomic_concerns", "synonyms_from_CCH", "synonymy_notes")))

Table_S1 %>% write_csv("data_Yosemite/Table_S1.csv", na = "")
```

```{r}
Table_S3 <- Yos_data %>%
  dplyr::filter(final_list == 1) %>%
  dplyr::select(
    accepted_name, family, broader, establishment_means,
    park_list = in_park_END,
    voucher_exists = voucher_END,
    iNat_RG_record_exists = iNat_RG_END
  )

Table_S3 %>% write_csv("data_Yosemite/Table_S3.csv", na = "")
```