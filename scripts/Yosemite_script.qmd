...
```{r}
Yosemite_list <- read_csv("data_Yosemite/Yosemite_all_data_2024.09.csv") %>%
...
```{r}
Yos_data <- read_csv("data_Yosemite/Yosemite_all_data_2024_09_22.csv")
```
...
voucher_outcomes_unique <- Yos_data %>%
  ...
  mutate(
    voucher_outcome = factor(voucher_outcome, levels = c("retain exact name", "retain species under synonymous name", "geographic error", "identification error", "taxonomic or nomenclatural error", "databasing discrepancy", "XX-synonym", "XX-not in graphics")),
    voucher_outcome_detailed = factor(voucher_outcome_detailed, levels = c(
      "retain exact name", "retain species under synonymous name",
      "remove, coordinates erroneously inside park",
      "add, coordinates erroneously outside park or missing coordinates",
      ...
      "XX-synonym", 
      "XX-no voucher, but occurs in park", 
      "XX-no voucher, not in park", 
      "XX-voucher with 'coordinates erroneously outside park or missing coordinates' found, but rejected due to 'incorrect identification'; not in park",
      "XX-voucher with 'coordinates erroneously outside park or missing coordinates' found, but rejected due to 'untraced taxonomic error'; not in park",
      "XX-voucher with 'coordinates erroneously outside park or missing coordinates' found, but rejected because 'horticultural record'; not in park",
...
``` {r}
 voucher_outcomes_collapsed <- Yos_data %>%
  ...
  mutate(
    outcome = factor(outcome, levels = c("retain exact name", "retain species under synonymous name", "geographic error", "identification error", "taxonomic or nomenclatural error", "databasing discrepancy"))
  ) %>%
  group_by(outcome) %>%
  mutate(outcome_counts = n()) %>%
  ungroup() %>%
...
### - parks list outcomes
```{r}
# list of park list outcomes
park_outcomes_unique <- Yos_data %>%
  distinct(parks_outcome, parks_outcome_detailed) %>%
  mutate(
    parks_outcome = factor(parks_outcome, levels = c("retain exact name", "retain species under synonymous name", "retain because vouchers for species are close to the park", "geographic error", "identification error", "taxonomic or nomenclatural error", "databasing discrepancy", "XX-synonym", "XX-not in graphics")),
    parks_outcome_detailed = factor(parks_outcome_detailed, levels = c(
      ...
      "vouchers missed due to undetected synonyms",
      "XX-synonym", 
      "XX-not on parks list, but occurs in park", 
      "XX-not on parks list, not in park",
      "XX-iNat needs ID only", "XX-Symbiotica only"
    ))
    ) %>%
...
parklist_outcomes <- Yos_data %>%
  ...
  filter(!stringr::str_starts(outcome, "XX")) %>%
  filter(!stringr::str_starts(outcome, "add")) %>%
  mutate(outcome = factor(outcome, levels = c("retain exact name", "retain species under synonymous name", "retain because vouchers for species are close to the park", "geographic error", "identification error", "taxonomic or nomenclatural error", "databasing discrepancy", "XX-synonym", "XX-not in graphics"))) %>%
  group_by(outcome) %>%
  mutate(outcome_counts = n()) %>%
  ungroup() %>%
  distinct() %>%
...
parklist_outcomes_collapsed <- Yos_data %>%
  ...
  filter(!stringr::str_starts(outcome, "XX")) %>%
  mutate(parks_outcome = factor(parks_outcome, levels = c("retain exact name", "retain species under synonymous name", "retain because vouchers for species are close to the park", "geographic error", "identification error", "taxonomic or nomenclatural error", "databasing discrepancy", "XX-synonym", "XX-not in graphics")),
    outcome = factor(outcome, levels = c(
      "retain exact name", "retain species under synonymous name",
      "retain because vouchers for species are close to the park",
      "remove, species range far from park",
...
iNat_outcomes_collapsed <- Yos_data %>%
  ...
  mutate(outcome = case_when(
    ...
    outcome == "retain, restricted & new weed record" ~ "2. retain restricted",
    outcome == "retain, restricted & new native record" ~ "2. retain restricted",
    outcome == "retain, auto-obscured coordinates" ~ "1. retain",
    outcome == "retain, manually obscured coordinates" ~ "1. retain",
    outcome == "remove, 01. wrong ID" ~ "4. identification errors",
    outcome == "remove, 04. misapplied name" ~ "4. identification errors",
    outcome == "remove, 02. wrong coordinates" ~ "3. geographic errors",
    outcome == "remove, 03. horticultural" ~ "7. databasing discrepancies",
